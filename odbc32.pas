// Title   : ODBC Delphi Header
// Version : V1.01.14

// 2003-NOV-28 Constants and basic functions. Dynamic Library Linking.
...
type
  ...
  SQLInteger   = Integer;
  SQLResult    = SQLSmallInt;

  TSQLAllocHandle = function(HandleType: DWord; InputHandle: DWord;
    var OutputHandle: DWord): SQLResult; stdcall;

  TSQLSetEnvAttr = function(EnvironmentHandle: DWord; Attribute: DWord;
    ValuePtr: DWord; StringLength: DWord): SQLResult; stdcall;

  TSQLDataSources = function(EnvironmentHandle: DWord; Direction: SQLSmallInt;
    ServerName: PChar; BufferLength1: SQLSmallInt; var NameLength1: SQLInteger;
    Description: PChar; BufferLength2: SQLSmallInt; var NameLength2: SQLInteger): SQLResult; stdcall;

  TSQLAllocConnect = function( EnvironmentHandle: DWord;
    ConnectionHandle: DWord ): SQLResult; stdcall;


  TSQLConnect = function(ConnectionHandle:DWord; pDSN: PChar; DSNLength: DWord;
    pUserName: PChar; NameLength: DWord; pPassword: PChar;
    PasswordLength: DWord): SQLResult; stdcall;

  TSQLDriverConnect = function(ConnectionHandle: DWord; hWnd:DWord;
    pInConnectString: PChar; InStringLength:DWord; pOutConnectString: PChar;
    OutBufferSize: DWord; var pOutConnectStringLength: DWord;
    DriverCompletion: DWord): SQLResult; stdcall;

  TSQLDisconnect = function(ConnectionHandle: DWord): SQLResult; stdcall;

  TSQLFreeHandle = function(HandleType: DWord; Handle: DWord): SQLResult; stdcall;

  TSQLExecDirect = function(StatementHandle:DWord; pStatementText: PChar;
    TextLength: DWord): SQLResult; stdcall;

  TSQLPrepare = function(StatementHandle:DWord; pStatementText: PChar;
    TextLength: DWord): SQLResult; stdcall;

  TSQLExecute = function(StatementHandle: DWord): SQLResult; stdcall;

  TSQLBindParameter = function(StatementHandle: DWord; ParameterNumber:DWord;
    InputOutputType: DWord; ValueType: DWord; ParameterType: DWord;
    ColumnSize: DWord; DecimalDigits: DWord; ParameterValuePtr: Pointer;
    BufferLength: DWord; pStrLenOrIndPtr: PDWord): SQLResult; stdcall;

  TSQLBindCol = function(StatementHandle: DWord; ColumnNumber: DWord;
    TargetType:DWord; TargetValuePtr: Pointer; BufferLength: DWord;
    var pStrLenOrIndPtr: DWord): SQLResult; stdcall;

  TSQLFetch = function(StatementHandle: DWord): SQLResult; stdcall;

  TSQLError = function(EnvironmentHandle: DWord; ConnectionHandle: DWord;
    StatementHandle: DWord; SqlState: PChar; var NativeError: SQLInteger;
    MessageText: PChar; BufferLength: SQLSmallInt; var TextLength: SQLInteger): SQLResult; stdcall;

  TSQLFreeStmt = function(StatementHandle: DWord; Option: SQLSmallInt): SQLResult; stdcall;

  TSQLRowCount = function(StatementHandle: DWord; var RowCount: SQLInteger): SQLResult; stdcall;

  TSQLColumns = function(StatementHandle: DWord; CatalogName: PChar; NameLength1: SQLSmallInt;
    SchemaName: PChar; NameLength2: SQLSmallInt; TableName:PChar; NameLength3: SQLSmallInt;
    ColumnName: PChar; NameLength4: SQLSmallInt): SQLResult; stdcall;

  TSQLTables = function(StatementHandle: DWord; CatalogName: PChar; NameLength1: SQLSmallInt;
    SchemaName: PChar; NameLength2: SQLSmallInt; TableName:PChar; NameLength3: SQLSmallInt;
    ColumnName: PChar; NameLength4: SQLSmallInt): SQLResult; stdcall;
  //

  TSQLInstallerError = function(iError: SQLInteger; var pfErrorCode: DWord;
    lpszErrorMsg: PChar; cbErrorMsgMax: SQLInteger;
    var pcbErrorMsg: SQLInteger): SQLResult; stdcall;

  TSQLInstallDriver = function(lpszDriver: PChar; lpszPathIn: PChar;
    lpszPathOut: PChar; cbPathOutMax: SQLInteger; var pcbPathOut: SQLInteger;
    fRequest: SQLInteger; lpdwUsageCount: DWord): boolean; stdcall;

  TSQLConfigDriver = function(hwndParent: DWord; fRequest: SQLInteger;
    lpszDriver: PChar; lpszArgs: PChar; lpszMsg: PChar; cbMsgMax: SQLInteger;
    var pcbMsgOut: SQLInteger): boolean; stdcall;

  TSQLCreateDataSource = function( hwndParent: DWord; lpszDS: PChar ): boolean; stdcall;

var
  // odbc
  SQLAllocHandle        : TSQLAllocHandle = nil;
  SQLSetEnvAttr         : TSQLSetEnvAttr = nil;
  SQLConnect            : TSQLConnect = nil;
  SQLDriverConnect      : TSQLDriverConnect = nil;
  SQLDisconnect         : TSQLDisconnect = nil;
  SQLFreeHandle         : TSQLFreeHandle = nil;
  SQLExecDirect         : TSQLExecDirect = nil;
  SQLPrepare            : TSQLPrepare = nil;
  SQLExecute            : TSQLExecute = nil;
  SQLBindParameter      : TSQLBindParameter = nil;
  SQLBindCol            : TSQLBindCol = nil;
  SQLFetch              : TSQLFetch = nil;
  SQLDataSources        : TSQLDataSources = nil;
  SQLError              : TSQLError = nil;
  SQLAllocConnect       : TSQLAllocConnect = nil;
  SQLFreeStmt           : TSQLFreeStmt = nil;
  SQLRowCount           : TSQLRowCount = nil;
  SQLColumns            : TSQLColumns = nil;
  // odbccp
  SQLInstallerError     : TSQLInstallerError = nil;
  SQLInstallDriver      : TSQLInstallDriver = nil;
  SQLConfigDriver       : TSQLConfigDriver = nil;
  SQLCreateDataSource   : TSQLCreateDataSource = nil;
  SQLTables             : TSQLTables = nil;
...
initialization
  ODBC_Lib := LoadLibrary('odbc32.dll');
  ODBCCP_Lib := LoadLibrary('c:\windows\system32\odbccp32.dll');
  if ODBC_Lib <> 0 then
  begin
    SQLAllocHandle      := GetProcAddress(ODBC_Lib,'SQLAllocHandle');
    SQLAllocConnect     := GetProcAddress(ODBC_Lib,'SQLAllocConnect');
    SQLSetEnvAttr       := GetProcAddress(ODBC_Lib,'SQLSetEnvAttr');

    SQLConnect          := GetProcAddress(ODBC_Lib,'SQLConnect');
    SQLDriverConnect    := GetProcAddress(ODBC_Lib,'SQLDriverConnect');

    SQLDisconnect       := GetProcAddress(ODBC_Lib,'SQLDisconnect');
    SQLFreeHandle       := GetProcAddress(ODBC_Lib,'SQLFreeHandle');

    SQLPrepare          := GetProcAddress(ODBC_Lib,'SQLPrepare');
    SQLExecute          := GetProcAddress(ODBC_Lib,'SQLExecute');
    SQLExecDirect       := GetProcAddress(ODBC_Lib,'SQLExecDirect');
    SQLFetch            := GetProcAddress(ODBC_Lib,'SQLFetch');
    SQLBindCol          := GetProcAddress(ODBC_Lib,'SQLBindCol');
    SQLBindParameter    := GetProcAddress(ODBC_Lib,'SQLBindParameter');

    SQLDataSources      := GetProcAddress(ODBC_Lib,'SQLDataSources');
    SQLError            := GetProcAddress(ODBC_Lib,'SQLError');
    SQLFreeStmt         := GetProcAddress(ODBC_Lib,'SQLFreeStmt');
    SQLRowCount         := GetProcAddress(ODBC_Lib,'SQLRowCount');
    SQLColumns          := GetProcAddress(ODBC_Lib,'SQLColumns');
    SQLTables           := GetProcAddress(ODBC_Lib,'SQLTables');
  end;

  if ODBCCP_Lib <> 0 then
  begin
    SQLInstallerError   := GetProcAddress(ODBCCP_Lib,'SQLInstallerError');
    SQLInstallDriver    := GetProcAddress(ODBCCP_Lib,'SQLInstallDriverEx');
    SQLConfigDriver     := GetProcAddress(ODBCCP_Lib,'SQLConfigDriver');
    SQLCreateDataSource := GetProcAddress(ODBCCP_Lib,'SQLCreateDataSource');
  end;
finalization
  if ODBC_Lib <> 0 then FreeLibrary(ODBC_Lib);
  if ODBCCP_Lib <> 0 then FreeLibrary(ODBCCP_Lib);
end.
