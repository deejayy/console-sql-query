...
type
  ...
  obj = ( oButton, oListBox, oMemo, oEdit, oShortCut, oComboBox, oDriveComboBox,
          ...
          oRadioButton, oFileListBox, oTimer );
...
procedure   tListBox.selfkbd( keycode: _KEY_EVENT_RECORD );
  ...
  begin
    ...
    case keycode.wVirtualKeyCode of
      ...
      VK_DOWN: if cursorpos < high( strings ) then setcrpos( 1, 0 );
...
procedure   tEdit.paint;
  ...
  begin
    ...
    if cursorpos = length( text ) then space := ' ' else space := '';
...
procedure   tMemo.paint;
  ...
  begin
    ...
    if cursorpos > high( strings ) - height then cursorpos := high( strings ) - height;
    if cursorpos < 0 then cursorpos := 0;
...
procedure   tMemo.selfkbd( keycode: _KEY_EVENT_RECORD );
  ...
  begin
    ...
    case keycode.wVirtualKeyCode of
      ...
      VK_DOWN: if cursorpos < high( strings ) - height then setcrpos( 1 );
...
procedure   tComboBox.paint;
  ...
  begin
    ...
    outstr := copy( strings[ itemindex ], 1, width );
